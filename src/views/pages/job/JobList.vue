        ...
        <div class="flex-grow">
          <vs-dropdown vs-trigger-click class="cursor-pointer">
            <div class="p-4 border border-solid d-theme-border-grey-light rounded-full d-theme-dark-bg cursor-pointer flex items-center justify-between font-medium">
              <span class="mr-2">{{ currentPage * paginationPageSize - (paginationPageSize - 1) }} - {{ jobsData.length - currentPage * paginationPageSize > 0 ? currentPage * paginationPageSize : jobsData.length }} z {{ jobsData.length }}</span>
...
<script>
  ...
  export default {
    ...
    created() {
      ...
      this.$store.dispatch("jobs/fetchAll").catch(err => { console.error(err) })
    }
  }
