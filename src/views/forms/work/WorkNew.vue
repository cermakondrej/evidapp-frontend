...
<script>
  ...
  export default {
    ...
    watch: {
      work: function () {
        if( this.work.workload !== null && (this.work.workload*1) > 0.75) {
          if(this.work.break_start === null){
            this.work.break_start = "12:00"
            this.work.break_end = "12:30"
          }
        }
      }
    },
      ...
      submitForm() {
        this.$validator.validateAll().then(result => {
          if (result) {
            ...
            this.$store.dispatch('works/create', payload)
              .then(() => {
                this.$vs.loading.close()
                this.$vs.notify({
                  color: 'success',
                  title: 'Pracovní úvazek přidán',
                  text: `Pracovní úvazek pro uživatele "${this.work.user.full_name}" byl úspěšně přidán.`
                })
              })
              .catch(error => {
                this.$vs.loading.close()
